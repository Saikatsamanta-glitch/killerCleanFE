...
//   return (
//     <div className="bg-white sm:mt-24 p-4  sm:px-20 sm:pt-20 w-full ">
...
import { Button } from "flowbite-react";
import { Link } from "react-router-dom";
import sectionData from "./sectionData"; // Import the data file
    ...
    <div className="bg-white sm:mt-24 p-4  sm:px-20 sm:pt-20 w-full ">
      ...
      {sectionData.map((section, index) => (
        <div key={index} className="flex items-center w-full">
          <div className={`w-[50%] flex flex-col items-${index % 2 === 0 ? "start" : "end"} p-8`}>
            <h1 className="text-4xl text-[#014584] mb-4 font-semibold">{section.title}</h1>
            <p className="text-[17px] text-[#014584] leading-8 text-left">{section.content}</p>
          </div>
          <div className={`w-[50%] flex justify-${index % 2 === 0 ? "end" : "start"}`}>
            <img src={section.image} alt="" className="w-[90%] p-7" />
          </div>
          {index === 1 && (
            <img
              src="https://example.com/overlay-image.png"
              alt=""
              className="absolute -left-[238px] top-20 h-52 -rotate-1"
            />
          )}
      ...
      <div className="flex w-full flex-row-reverse items-center relative">
        {/* Remaining code for the last section */}
      </div>
    </div>
  );
}
